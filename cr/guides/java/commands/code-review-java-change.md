...
`.code-review.yml` 按最小配置规范解释，支持项目架构、忽略路径、规则严重级别覆盖、验证命令和报告输出配置。配置规范见 `cr/guides/java/config/code-review-yml.md`。
...
优先按规则卡片输出问题。规则卡片格式见 `cr/guides/java/rules/README.md`。
...
JSON 格式遵循 `cr/guides/java/report/json-report.md`。如果写文件，默认路径：
