...
> 本文属于 [Mod 化 / 插件化设计总览](think.md) 中的“具体实现”维度，用来说明移动端为什么更适合受控模块化、轻量 UI 插槽、配置控制能力开放和复杂能力服务端承载。
...
> 移动端插件化应该是“受控模块化”，即 App 本体保持轻量稳定，能力通过内置模块、配置开关、场景包、服务端接口和受限插槽扩展，而不是让用户或第三方随意安装和运行插件。

可以抽象为：

```text
移动端插件化 = App 本体 Core + 受控能力模块 Modules + 轻量 UI 插槽 Slots + 配置控制 Config + 服务端能力承载 Services
```
...
flowchart TB
    ...
    App --> Config[配置控制<br/>灰度 / AB / 权益 / 活动 / 降级]
    App --> Services[服务端能力承载<br/>搜索 / 推荐 / 内容处理 / 数据分析]
...
| 服务端能力承载      | 复杂能力主要在服务端执行，端上只承载入口、授权、请求和结果展示 | 搜索、推荐、内容处理、数据分析 |
...
这里的远程配置中心，本质上是用来控制能力开放规则：谁能用、什么版本能用、什么时候展示、异常时如何降级。

### 8.4 复杂能力服务端承载

- 搜索、推荐、内容处理、数据分析等复杂能力尽量由服务端承载。
- App 端只承载入口、授权、展示和交互。
- 降低包体和端上计算压力。
